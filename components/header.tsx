        ...
        <header className='z-[999] relative'>
            ...
            <nav
                ...
                <ul className='flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5'>
                    {links.map(l => (
                        <motion.li key={l.hash} className='h-3/4 flex items-center justify-center relative'
                                   initial={{y: -100, opacity: 0}} animate={{y: 0, opacity: 1}}
                        >
                            <Link className={clsx(`flex w-full items-center
        justify-center px-3 py-3 hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300`,
                                {'text-gray-950 dark:text-gray-200': activeSection === l.name})}
                                  href={l.hash}
                                  onClick={() => {
                                      setActiveSection(l.name)
                                      setTimeOfLastClick(Date.now())
                                  }}
                            >
                                {l.name}
                                {l.name === activeSection && (
